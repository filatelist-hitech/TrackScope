...
**Результат:** KeyAnalyzer корректно определяет D major → 10B, G minor → 6A, E minor → 9A, Bb major → 6B. Белый шум больше не даёт систематически 4A/4B.

**Полное покрытие 24 тональностей (review-gate, 2026-06-05):** добавлены data-driven тесты `all_24_keys_detect_correct_camelot`, `all_24_keys_detect_correct_mode`, `all_24_keys_have_confidence_above_threshold` в `core/dsp/tests/key_detection.rs`. Все 24 тональности (12 major + 12 minor) детектируются корректно с confidence 0.89–0.94. Регрессионные тесты key_detection: 22/22 pass.

**Замечание о неоднозначности bare-триад:** голая мажорная триада {R, M3, P5} музыкально неоднозначна с относительным минором (C мажор {C,E,G} = тоника/m3/m6 ноты E минора), и K-S корреляция на 3 нотах может выбрать относительный минор. Тесты используют tonic-emphasized триады (тоника ×2), что моделирует роль баса/тонального центра в реальной музыке и разрешает неоднозначность. Минорные триады детектируются корректно даже без акцента (минорная триада однозначно пикует минорный профиль). На реальных треках тональный центр задаётся басом и мелодией, поэтому эта неоднозначность не проявляется.
