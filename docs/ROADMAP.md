...
- 1/f-нормализация в HPCP — **устранена в Phase 13.2** (заменена Gaussian weighting).
- Ручная верификация на ≥5 реальных треков с известным ключом рекомендована после Phase 13.2.

---

## Phase 13.2: KeyAnalyzer accuracy — Gaussian HPCP + Temperley профили — **ЗАВЕРШЕНО** (2026-06-05)

Цель: устранить систематическую ошибку KeyAnalyzer, из-за которой детектор выдавал
почти исключительно 4A (F minor) или 4B (Ab major) вне зависимости от реального ключа.

### Диагноз root cause

Три взаимодействующие причины:

| # | Причина | Эффект |
|---|---|---|
| 1 | 1/f-нормализация: `power / freq` | Первый бин (k=9, ~105 Hz) = Ab, вес 0.00948 — в 50× больше высоких частот |
| 2 | FREQ_MIN=100 Hz | Бин 105 Hz (Ab) имеет максимальный 1/f-вес → Ab систематически раздут (+1.70%) |
| 3 | K-S MINOR_PROFILE[3]=5.38 | У F minor (root=5) minor-3rd = Ab → двойной буст → 4A почти всегда |

Измеренный Ab-bias при равномерном шуме: **+1.70%** сверх идеальных 8.33%.

### Фикс

- **Убрана 1/f-нормализация** → вес бина = реальная мощность `buf[k].norm_sqr()`
- **Gaussian weighting σ=25 центов** (Gómez 2006) вместо жёсткого round()
- **FREQ_MIN**: 100 → **200 Hz** — исключает фундаменталы баслайна (80–200 Hz)
- **Temperley (2001) профили** добавлены параллельно с K-S (1990): всего 48 корреляций вместо 24

### Тесты

| Новый тест | Результат |
|---|---|
| `hpcp_is_approximately_flat_for_uniform_spectrum` | max deviation < 4% (до фикса: >2.2%) |
...
- Без count-normalization структурный bias при равномерном шуме: D/Db ~11% вместо 8.33% (~2.7% max dev). На реальных треках сигнал доминирует.
- Gaussian σ=25 cents слегка смягчает дискретизацию (не решает её полностью при бинах 40–80 центов в диапазоне 200–500 Hz).
- Проверка на широкой выборке реальных треков с известным ключом (≥5) рекомендована.
