...
#### 4.3 Реальные тестовые записи — **НЕ НАЧАТО**
...
- Расширение `tools/offline-lab/parity.py` для прогона на реальных WAV-записях — **НЕ ЗАВЕРШЕНО** (ждёт задачу 4.3).
- Допуск ±4 BPM для шумных реальных фикстур — **НЕ ЗАВЕРШЕНО** (ждёт задачу 4.3).
- Python-тест на реальном треке 200 BPM — **НЕ ЗАВЕРШЕНО** (ждёт задачу 4.3).
